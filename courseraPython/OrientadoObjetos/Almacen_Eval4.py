...
class ProductoSinPrecio(Exception):
    pass


class CompraPendiente(Exception):
    pass


class CompraTerminada(Exception):
    pass


class CompraNula(Exception):
    pass

class FaltantePago(Exception):
    pass
...
class PriceList(object):
    ...
    def price_of(self, a_product):
        if not a_product in self.price_of_products.keys():
            raise ProductoSinPrecio("Producto {} Sin Precio Definido.".format(a_product.code()))
...
class Purchase(object):
    ...
    def finish(self):
        if self.finished:
            raise CompraTerminada("Esta Compra Ha Sido Finalizada")
        if not self._products:
            raise CompraNula("Esta Compra No Contiene Productos")
        self.finished = True
...
class CashRegister(object):
    ...
    def _scan_product(self, a_product_code):
        ...
        if not selected_products:
            raise ProductoNoExiste("El Producto Con CÃ³digo {} No Existe".format(a_product_code))
    ...
    def total(self):
        if not self.current_purchase().finished:
            raise CompraPendiente("Esta Compra No Esta Concluida")
        total = 0
        for product in self.current_purchase().products():
    ...
    def pay_purchase_with(self, amount_of_money):
        ...
        if total > amount_of_money:
            raise FaltantePago("El Pago Debe Ser Mayor o Igual Al Total De La Compra.")
...
class CashRegisterTest(unittest.TestCase):
    ...
    def test_01_add_not_exist_product(self):
        ...
        self.assertRaises(ProductoNoExiste, cash_register.add_product, a_product_code)
    ...
    def test_04_subtotal_product_without_price(self):
        ...
        self.assertRaises(ProductoSinPrecio, self.cash_register.subtotal)
    ...
    def test_07_total_without_finish_purchase(self):
        ...
        self.assertRaises(CompraPendiente, self.cash_register.total)

    def test_08_finish_empty_purchase(self):
        self.assertRaises(CompraNula, self.cash_register.finish_purchase)
    ...
    def test_09_finish_purchase_already_finished(self):
        ...
        self.assertRaises(CompraTerminada, self.cash_register.finish_purchase)
    ...
    def test_13_pay_purchase_not_enough_money(self):
        ...
        self.assertRaises(FaltantePago, self.cash_register.pay_purchase_with, 13.00)
